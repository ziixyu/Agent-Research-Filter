...
# Prior epistemic credence P(E), keyed by the *same* StudyDesign literal the
# LLM already extracts (schema unchanged — see PaperTelemetry below). Values
# are anchored to the standard evidence-hierarchy tiers:
PRIOR_CREDENCE = {
    "Meta-Analysis": 0.95,  # meta-analysis / systematic review tier
    # Our extraction schema (deliberately) doesn't capture blinding or trial
    # phase, so "RCT" can't be split into double-blind Phase III (0.90) vs
    # open-label Phase II (0.70) the way a richer schema could. We default to
    # the stricter (Phase III) end rather than guess, and rely on the
    # Discrepancy Index below to catch RCTs whose *claims* overreach what
    # their actual design supports — see "known limitation" in README.md.
    "RCT": 0.90,
    "Prospective Cohort": 0.55,  # interpolated: less recall/selection bias
    # than retrospective work, but well short of a randomized design
    "Retrospective/Observational": 0.40,  # cohort/observational tier
    "In-Vitro/Animal": 0.20,  # case series / in-vitro / in-silico tier
    "Review/Opinion": 0.15,  # below in-vitro: synthesizes no new primary data
}
...
def score_papers(
    ...
    for (meta, tele), v_raw, v_norm, w_n in zip(relevant, raw_velocities, velocity_norms, power_norms):
        prior = PRIOR_CREDENCE[tele.study_design]
        rigor_baseline = prior * 5.0

        # Discrepancy Index D: how far the claim overshoots what this
        # evidence tier can justify. Underclaiming (D < 0) is never
        # penalized — caution is free.
        raw_d = tele.claim_hyperbole - rigor_baseline
        d = max(0.0, raw_d)

        # Sample Power Weight moderates the discrepancy: the same overclaim
        # from a large, well-powered study is less absurd than the identical
        # overclaim from a tiny one. Multiplier ranges [1.0, 2.0] — the
        # highest-power paper in the batch gets no amplification, the
        # lowest-power paper gets its discrepancy doubled.
        ...
        scored.append(
            ScoredPaper(
                ...
                rigor_baseline=round(rigor_baseline, 4),
                discrepancy_index=round(d, 4),
                sample_power_weight=round(w_n, 4),
                discrepancy_adjusted=round(d_adjusted, 4),
                likelihood_penalty=round(likelihood, 4),
